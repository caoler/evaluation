...
import cn.caoler.evaluation.converter.Index2IndexNodeConverter;
...
import static cn.caoler.evaluation.converter.Index2IndexNodeConverter.convert;

/**
 * Created by caole on 2019/4/8 21:02
 */
@Service
@Transactional
public class IndexTreeServiceImpl implements IndexTreeService {
    ...
    public IndexNode tree(IndexNode indexNode) {
        ...
        List<IndexNode> indexNodeList = Index2IndexNodeConverter.convert(indexList);
...
//         List<IndexNode> leafNodeList = new ArrayList<>();        //装叶子结点，因为只有叶子结点才会作为题目      (已经搞成了全局静态变量)
    ...
    public List<IndexNode> getParentNode(IndexNode indexNode) {
        List<Index> indexList = indexService.findAll();
        List<IndexNode> allIndexNodeList = Index2IndexNodeConverter.convert(indexList);    //把所有指标放进容器中
//        List<IndexNode> parentNodes = new ArrayList<>();        //用来装所有的父节点   此变量需要声明成静态变量
    ...
    public IndexNode formTreeByRole(Long roleId) {
        ...
        for ( Index index : indexList){
            indexNodeList.add(Index2IndexNodeConverter.convert(index));      //把某个角色的所有指标放进容器中
        }
